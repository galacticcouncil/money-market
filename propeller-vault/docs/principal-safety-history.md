# Principal Safety: Historical Design and Verification

**Archive, retained on 23 September 2026.** This records the design discussion
and dated verification checkpoints. Statements such as "local", "latest" and
"not published" refer to those checkpoints, not the present branch status.
Temporary paths identify historical work products and may not be available to
other contributors. Do not use this file as a deployment procedure.

Use [current principal policy](principal-safety.md) and the
[RC verification and activation gates](release-candidate.md) for team review.

For the new Main-interest policy, debt-cohort accounting and current verification,
see [yield-funded Main servicing](main-debt-servicing.md). Sponsored buffers and
their verification report are superseded, not production evidence for this revision.
...
AFTER start is assigned to the exit's own settlement cash and live debt units; it
cannot consume remaining holders' cash. See [Main servicing](main-debt-servicing.md).

...
Operational scripts for running, configuring, and verifying the BIL deployment on Hydration's `lark2` test chain. Mainnet uses the `bil-launch` hardhat task instead — these scripts are testnet-only.
...
ts-node scripts/bil/whitelist-deployer.ts

# 2. Fund the deployer with HDX
ts-node scripts/bil/fund-account.ts

# 3. Deploy core + BIL market
MARKET_NAME=BIL HARDHAT_NETWORK=lark2 npx hardhat deploy --tags market

# 4. Deploy LockableAToken impl + FixedPriceOracle for stHDX
MARKET_NAME=BIL HARDHAT_NETWORK=lark2 npx hardhat deploy-LockableAToken
...
HARDHAT_NETWORK=lark2 npx hardhat run scripts/bil/transfer-admin-to-governance.ts

# 6. Wire up stHDX oracle via TC referendum
ts-node scripts/bil/set-sthdx-oracle.ts

# 7. Submit the BIL init proposal via TC referendum
ts-node scripts/bil/submit-bil-proposal.ts

# 8. Point the runtime's BIL adapter at the new pool
ts-node scripts/bil/set-bil-pool.ts

# 9. Sanity check the whole thing
ts-node scripts/bil/generate-addresses.ts        # writes deployments/lark2/_addresses.{json,md}
ts-node scripts/bil/verify-readiness.ts          # PASS/FAIL table

# 10. End-to-end smoke test
ts-node scripts/bil/test-e2e.ts
```
...
The lark scripts intentionally bypass `generateProposalV2` and submit raw substrate calls one at a time. With Alice as the only TC member, this is dramatically faster than going through the full preimage/decoder pipeline that `tasks/proposals/bil-launch.ts` uses for mainnet. For the same reason, do **not** copy this pattern to mainnet — mainnet must go through proper governance.

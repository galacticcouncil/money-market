// Transfer BIL admin roles from our deployer to Hydration's governance
...
//   - PoolAddressesProvider-BIL
//     * setACLAdmin(governance)
//     * transferOwnership(governance)
//   - AaveOracle-BIL is owned by PoolAddressesProvider (no separate ownership)
...
// of every role afterwards by scripts/bil/revoke-deployer.ts (the final
...
async function main() {
  ...
  const aclArtifact = await hre.deployments.get("ACLManager-BIL");
  ...
  console.log("=== ACLManager-BIL roles ===");
  ...
  console.log("\n=== PoolAddressesProvider-BIL ===");
  const papArtifact = await hre.deployments.get("PoolAddressesProvider-BIL");

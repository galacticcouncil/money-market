// LIQUIDATION test for BIL (missing from test-e2e.ts which only covers
// supply/borrow/repay/withdraw/transfer).
//
// Goal: exercise AAVE V3's Pool.liquidationCall against the BIL pool with
...
//      prior bil state from existing borrowers).
...
// The catch: Alice's stHDX is lock-bound (Stakes.bil > 0). A direct EVM

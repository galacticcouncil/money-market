...
//   - HOLLAR underlying token = single shared ERC20 (no -GIGAHDX postfix)
//   - GhoAToken (HOLLAR's per-MM Aave wrapper) IS GIGAHDX-specific
//   - The proxy deployed for HOLLAR in the GIGAHDX MM uses the
//     GhoAToken-GIGAHDX implementation that we deployed in Phase 4
...
async function main() {
  ...
  // ---------- 2. HOLLAR's wrappers in the GIGAHDX pool ----------
  ...
  console.log("\n" + "=".repeat(70));
  console.log("  HOLLAR wrappers in the GIGAHDX pool — these ARE GIGAHDX-specific");
  console.log("=".repeat(70));
  ...
  // Read each token's symbol — these have the GIGAHDX prefix
  ...
  const artifactPath = "deployments/lark2/GhoAToken-GIGAHDX.json";
  if (fs.existsSync(artifactPath)) {
    ...
    console.log(`  → the proxy delegates to GhoAToken-GIGAHDX implementation`);
  }
  ...
  const vdArtifactPath = "deployments/lark2/GhoVariableDebtToken-GIGAHDX.json";
  if (fs.existsSync(vdArtifactPath)) {
    ...
    console.log(`  → the proxy delegates to GhoVariableDebtToken-GIGAHDX implementation`);
  }
  ...
  console.log(`  HOLLAR-GIGAHDX wrappers:       3 proxies × 1 GIGAHDX-specific impl each`);

// Recovery proposal for the GIGAHDX HOLLAR-side cross-refs that the
// submit-gigahdx-proposal.ts batch failed to set due to gotcha #10
...
async function main() {
  ...
  const ghoATokenAbi = (await hhre.deployments.get("GhoAToken-GIGAHDX")).abi;
  const ghoVarDebtAbi = (await hhre.deployments.get("GhoVariableDebtToken-GIGAHDX")).abi;

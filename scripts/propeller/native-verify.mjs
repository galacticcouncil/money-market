...
for (const deployment of result.deployments) {
  const name = deployment.label.endsWith('.proxy') ? 'ERC1967Proxy'
    : deployment.label.includes('OperatingBuffer') ? 'PropellerOperatingBuffer' : deployment.label;
...
const buffer = await pub.readContract({ address: checkedVault, abi: artifact('CollateralVault').abi, functionName: 'operatingBuffer' });
const bufferRead = functionName => pub.readContract({address: buffer, abi: artifact('PropellerOperatingBuffer').abi, functionName});
...
assert.ok(await bufferRead('coverageSeconds') > 0);
assert.ok(await bufferRead('exitCostBps') > 0);
...
assert.ok(backing >= await bufferRead('ownedCash') + await bufferRead('bootstrapCash'));
...
for (let id = 0n; id < exitCount; id++) {
  const position = await pub.readContract({address: buffer, abi: artifact('PropellerOperatingBuffer').abi, functionName:'positions', args:[id + 1n]});
  ...
  pending += position[3];
  if (!result.finalBufferExits.some(x => x.vault === checkedVault && x.id === id.toString())) {
...
assert.equal(pending + await bufferRead('activeSourceRemaining'), sourcePending);
...
result.checks.exitOwnershipAndPendingSourceClaims = true;
result.verifiedThroughBlock = (await pub.getBlockNumber()).toString();
writeFileSync(FILE, JSON.stringify(result, null, 2) + '\n');
console.log('FINAL VERIFICATION PASS', result.status, result.checks);

...
task(`review-e-mode`, ``)
  .addFlag("fix")
  .addFlag("batch")
  .addParam("name", "name of EMode Category")
  .setAction(
    async (
      { name, fix, batch }: { name: string; fix: boolean; batch: boolean },
      hre
    ) => {
      ...
      const configEmodeCategory = poolConfig.EModes[name];
      const id = configEmodeCategory.id;
      ...
      if (delta) {
        ...
        console.log("  - Updating EMode Category", id);
